...
public interface DocumentChunkRepository extends JpaRepository<DocumentChunk, UUID> {
    ...
    @Query(value = """
        ...
        WHERE d.status = 'COMPLETED' 
        AND dc.embedding IS NOT NULL 
        AND (dc.content ILIKE %:keyword% OR to_tsvector('english', dc.content) @@ plainto_tsquery('english', :keyword))
        ORDER BY 
    ...
    @Query(value = "SELECT * FROM document_chunk WHERE content ILIKE %:keyword% ORDER BY chunk_index LIMIT :limit", nativeQuery = true)
    List<DocumentChunk> findByContentContaining(@Param("keyword") String keyword, @Param("limit") int limit);

    @Query("SELECT dc FROM DocumentChunk dc JOIN dc.document d WHERE d.status = 'COMPLETED' AND dc.content ILIKE %:keyword%")
